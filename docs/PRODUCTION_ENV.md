...
Both browsers call the relative path `/api/v1`, so the Next server forwards everything. Each app has **two** forwarders, and since September 2026 they divide the work instead of competing for it: the route handler (`src/app/api/v1/[...path]/route.ts`) serves every `/api/v1/*` call and relays the visitor's address for per-person rate limiting (M-11); the `next.config.ts` rewrite serves only `/api/v1/uploads/*` — in the client everything there except `signup-doc`, which is unauthenticated and capped at 3 MB and so goes through the handler — plus the static `/uploads/*` files. Before that, a blanket `/api/v1/:path*` rewrite matched first everywhere (Next checks afterFiles rewrites before dynamic routes), so the handler never ran and the relay was never sent. A response served by the handler carries `x-lms-proxy: handler`; one served by the rewrite carries nothing.

All four forwarders now resolve the backend origin from the **same three names in the same order**, with the same fallback:

| Where | Chain | Default |
|---|---|---|
| `client/next.config.ts` (upload rewrite), `client/src/app/api/v1/[...path]/route.ts` (handler), `admin/next.config.ts` (upload rewrite), `admin/src/app/api/v1/[...path]/route.ts` (handler) | `API_URL` → `NEXT_PUBLIC_API_URL` → `NEXT_PUBLIC_API_BASE_URL` | `http://localhost:8000` |

They used to differ — four orders, two fallback ports — which is how copying one app's `.env` to the other could break the one you copied into. Any one of the three names now works in both apps. **Setting all three to the same value is still the recommendation**: the `NEXT_PUBLIC_` ones are frozen into the bundle at build time and `API_URL` is read at run time, so a project that sets only one of them can still drift between a build and a redeploy.

| Variable | Required? | Production value | What breaks if wrong |
|---|---|---|---|
| `API_URL` (both apps) | Has a default, so the app boots — runtime | `https://api-lms.deltainstitutions.com` — **bare origin, no trailing slash, and absolutely no `/api/v1` suffix** (both proxies append it) | Wrong → every API call is forwarded to a dead port; the handler answers 502 `PROXY_ERROR`. Both handlers read it per request, so a changed value needs a redeploy (or a restart of a self-hosted Next server) and nothing more; the upload rewrites bake it in at build time |
| `NEXT_PUBLIC_API_URL` (both) | **Build-time** | same value | Second choice in every chain, and the source for the client's server-rendered `/courses/by-id/[id]` page, so a wrong value dead-ends ObjectId links while slug links keep working. It is `NEXT_PUBLIC_`, so it is inlined into the public bundle — if you optimise `API_URL` to a loopback address, keep this one on the public https host |
| `NEXT_PUBLIC_API_BASE_URL` (both) | **Build-time** | same value | Third choice in every chain. With all three missing, every forwarder lands on `localhost:8000` and fails loudly with 502 `PROXY_ERROR` — no longer the `localhost:4000` the client rewrite used to fall back to, which on the production box is the **exam-tracker API** and would have served another product's 404s |
| `PROXY_SHARED_SECRET` (both) | Runtime feature | the **same 64 hex** as `backend/.env` | Unset → the relay headers are simply not sent (the `pickClientIp` block in each `route.ts`) and all users share one rate-limit bucket. Until September 2026 this was true regardless of the value, because the rewrite shadowed the handler and the header could never be sent at all. Set-but-mismatched is worse: identical symptom while you believe it is fixed. **The frontends do not trim; the backend does** — a trailing space is a silent mismatch |
