 ...
 * visitor shared one rate-limit bucket, and the backend said so on every boot
 * ("NO request has ever presented it"). The rewrite now covers only the
 * large authenticated uploads — next.config.ts says why they stay there — and
 * the static /uploads files.
 *
...
/* The SAME three names, in the same order, as next.config.ts and the admin
   app. This handler used to read API_URL → NEXT_PUBLIC_API_BASE_URL while the
   rewrite that actually served traffic read NEXT_PUBLIC_API_URL first, so a
   deployment configured for the rewrite would have resolved localhost here
   the moment the handler went live. Read per request, so a changed value
   needs a redeploy and nothing more. */
const backendOrigin = () =>
...
/* Vercel stops a function at the plan's default (10–15 s) unless told
   otherwise. The signup upload carries 3 MB from a phone and the form gives
   the register call 45 s; 60 is inside every plan's ceiling. */
...
async function proxy(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    /* Sentences, not stack traces: the forms put error.message in front of
       the student verbatim, and "TypeError: fetch failed" is not for them. */
